...
class Program
{
    static void Main(string[] args)
    {
        ...
        var command = ParseEnum<VOLUME>(args[0]);
        runCommand(command, args);

    }


    static string runCommand(VOLUME command, string[] args)
    {
        return command switch
        {
            VOLUME.AUMENTA => aumenta(),
            VOLUME.DIMINUI => diminui(),
            VOLUME.MAXIMO => byParameter("100"),
            VOLUME.MINIMO => byParameter("0"),
            _ => byParameter(args[0])
        };
    }

    static string aumenta()
    {
        var currentVolume = VideoPlayerController.AudioManager.GetMasterVolume();
        float changeVolume = currentVolume;
        if (currentVolume + 10 > 100) { changeVolume = 100; }
        else { changeVolume = changeVolume + 10; }
        VideoPlayerController.AudioManager.SetMasterVolume(changeVolume);
        return currentVolume.ToString();
    }

    static string diminui()
    {
        var currentVolume = VideoPlayerController.AudioManager.GetMasterVolume();
        float changeVolume = currentVolume;
        if (currentVolume - 10 < 0) { changeVolume = 0; }
        else { changeVolume = changeVolume - 10; }
        VideoPlayerController.AudioManager.SetMasterVolume(changeVolume);
        return currentVolume.ToString();
    }

    static string byParameter(string arg)
    {
        var currentVolume = VideoPlayerController.AudioManager.GetMasterVolume();
        VideoPlayerController.AudioManager.SetMasterVolume(float.Parse(arg));
        return currentVolume.ToString();
    }
